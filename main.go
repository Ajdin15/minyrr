package main
	...
	func convert() {
		...
		for {
			...
			if buffer[0] == 0x0A {
				...
				if lineCount == 16756 {
					newLastLine := "Data er basert paa gyldig data (per 18.03.2023) (CC BY 4.0) fra Meteorologisk institutt (MET);endringen er gjort av Ajdin Smajic"
				...
				if len(elementArray) > 3 {
					...
					fahr := conv.CelsiusToFarhenheit(f)
	...
	func average(unit string) {
		...
		if unit == "c" {
			...
			fahr := conv.CelsiusToFarhenheit(average)
			fmt.Printf("%.2f\n", fahr)
		}
